...
public class ISHR extends gov.nasa.jpf.jvm.bytecode.ISHR {
	...
	public Instruction execute(ThreadInfo ti) {
		...
		if(sym_v1 == null && sym_v2 == null) {
			...
			if(sym_v1 != null) {
				if (sym_v2 != null) {
					result = new IntegerBinaryExpression(Operator.SHIFTR, sym_v1, sym_v2);
				} else {
					result = new IntegerBinaryExpression(Operator.SHIFTR, sym_v1, new IntegerLiteral(v2));
				}
			} else {
				result = new IntegerBinaryExpression(Operator.SHIFTR, new IntegerLiteral(v1), sym_v2);
			}
